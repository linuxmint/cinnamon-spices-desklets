...
class StopwatchDesklet extends Desklet.Desklet {
  constructor(metadata, deskletId) {
    ...
    this.colorLabel =
      this.settings.getValue("colorLabel") || "rgb(51, 209, 122)";
    this._timeout = null;
    this._startTime = 0;
    this._elapsedTime = 0;
  ...
  _setupLayout() {
    ...
    const playIcon = this._getImageAtScale(
      `${this.metadata.path}/play.svg`,
      computedHeight,
      computedHeight
    );
    ...
    const pauseIcon = this._getImageAtScale(
      `${this.metadata.path}/pause.svg`,
      computedHeight,
      computedHeight
    );
    ...
    const stopIcon = this._getImageAtScale(
      `${this.metadata.path}/stop.svg`,
      computedHeight,
      computedHeight
    );
    ...
    this.buttonRow = new St.BoxLayout({ style_class: "button-row" });
    ...
    this.contentLayout = new St.BoxLayout({
      ...
      style_class: "content-layout",
    });
    this.contentLayout.add_child(this.buttonRow);
    this.contentLayout.add_child(this.timeLabel);
  ...
  _createLabel(text, color = "inherit") {
    return new St.Label({
      ...
      style: `font-size: ${this.fontSize}px; color: ${color}; vertical-align: middle;`,
    });
  }
  ...
  _getImageAtScale(imageFileName, requestedWidth, requestedHeight) {
    try {
      ...
      const actor = new Clutter.Actor({ width, height }); // Verwenden Sie die tatsächliche Breite und Höhe des Pixbuf
      actor.set_content(image);
      return actor;
    } catch (e) {
      global.logError(`Fehler beim Laden des Bildes ${imageFileName}: ${e}`);
  ...
  _updateTime() {
    ...
    const elapsedTime =
      this._elapsedTime + (this._isRunning ? currentTime - this._startTime : 0);
