...
const Calendars    = imports.calendars.Calendars;
...
CalendariumDesklet.prototype = {
    ...
    _bindAllSettings: function(desklet_id) {
        ...
        s.bind("show-moonrise",  "show_moonrise",  cb);

        // Sun
        s.bind("show-sun",      "show_sun",      cb);
        s.bind("show-solstice", "show_solstice", cb);
        ...
        s.bind("show-period-upcoming",    "show_period_upcoming",    cb);
        s.bind("period-upcoming-lookahead","period_upcoming_lookahead", cb);
        ...
        // Alternate calendars
        s.bind("show-julian",  "show_julian",  cb);
    ...
    _setupUI: function() {
        ...
        // ── Active seasonal periods ────────────────────────────────────────
        this._labelPeriod = new St.Label({
            style_class: "calendarium-period",
            text:        ""
        });
        this._container.add_actor(this._labelPeriod);

        // ── Upcoming seasonal periods ──────────────────────────────────────
        this._labelPeriodUpcoming = new St.Label({
            style_class: "calendarium-period-upcoming",
            text:        ""
        });
        this._container.add_actor(this._labelPeriodUpcoming);
        ...
        // ── Moon rise & set ────────────────────────────────────────────────
        this._moonRiseRow = new St.BoxLayout({
            vertical:    false,
            style_class: "calendarium-moontime-row"
        });

        this._labelMoonrise = new St.Label({
            reactive:    true,
            style_class: "calendarium-moontime",
            text:        ""
        });
        this._labelMoonrise.accessible_name = _("Moonrise");
        this._moonriseTooltip = new Tooltips.Tooltip(this._labelMoonrise, _("Moonrise"));

        this._labelMoonset = new St.Label({
            reactive:    true,
            style_class: "calendarium-moontime",
            text:        ""
        });
        this._labelMoonset.accessible_name = _("Moonset");
        this._moonsetTooltip = new Tooltips.Tooltip(this._labelMoonset, _("Moonset"));

        this._moonRiseRow.add_actor(this._labelMoonrise);
        this._moonRiseRow.add_actor(this._labelMoonset);
        this._container.add_actor(this._moonRiseRow);
        ...
        // ── Equinox / solstice ─────────────────────────────────────────────
        this._labelSolstice = new St.Label({
            style_class: "calendarium-solstice",
            text:        ""
        });
        this._labelSolstice.accessible_name = _("Equinox and Solstice");
        this._container.add_actor(this._labelSolstice);
        ...
        this._labelAltCal = new St.Label({
            style_class: "calendarium-altcal",
            text:        ""
        });
        this._labelAltCal.accessible_name = _("Alternate Calendars");
        this._container.add_actor(this._labelAltCal);
    ...
    _refresh: function() {
        ...
        try { this._updateMoonTimes(now);   } catch(e) { global.logError("Calendarium _updateMoonTimes: "   + e); }
        try { this._updateSun(now);         } catch(e) { global.logError("Calendarium _updateSun: "         + e); }
        try { this._updateSolstice(now);    } catch(e) { global.logError("Calendarium _updateSolstice: "    + e); }
        ...
        try { this._updateAltCal(now);      } catch(e) { global.logError("Calendarium _updateAltCal: "      + e); }
    ...
    _updateHoliday: function(now) {
        ...
        // ── Active seasonal periods ───────────────────────────────────────
        let periods = this.show_holidays
            ? Holidays.getPeriodsForDate(this._holidayData, now)
            : [];
        if (periods.length === 0) {
            this._labelPeriod.visible = false;
        } else {
            let plines = periods.map(function(p) {
                return "\u{1F4C5} " + p.name + " \u00b7 " + p.daysLeft + " " + _("days left");
            });
            this._labelPeriod.set_text(plines.join("\n"));
            this._labelPeriod.visible = true;
        }

        // ── Upcoming seasonal periods ─────────────────────────────────────
        let lookaheadPeriods = this.period_upcoming_lookahead || 30;
        if (!this.show_period_upcoming) {
            this._labelPeriodUpcoming.visible = false;
        } else {
            let upcoming = Holidays.getUpcomingPeriods(this._holidayData, now, lookaheadPeriods);
            if (upcoming.length === 0) {
                this._labelPeriodUpcoming.visible = false;
            } else {
                let ulines = upcoming.map(function(p) {
                    return "\u25b6 " + p.name + " \u00b7 " + p.daysUntil + " " + _("days");
                });
                this._labelPeriodUpcoming.set_text(ulines.join("\n"));
                this._labelPeriodUpcoming.visible = true;
            }
        }
    },
    ...
    _updateMoonTimes: function(now) {
        ...
        this._labelMoonrise.set_text("\u263E\u2191 " + riseStr);
        this._labelMoonset.set_text( "\u263E\u2193 " + setStr);
        this._moonriseTooltip.set_text(_("Moonrise") + ": " + riseStr);
        this._moonsetTooltip.set_text( _("Moonset")  + ": " + setStr);
    },

    _updateSolstice: function(now) {
        this._labelSolstice.visible = this.show_solstice;
        if (!this.show_solstice) return;

        let ev = Solstice.getNext(now);
        if (!ev) { this._labelSolstice.visible = false; return; }
        ...
        if (ev.daysUntil === 0) {
            ...
            text = "\u2600 " + name + " \u00b7 " + ev.daysUntil + " " + _("days");
        }
        this._labelSolstice.set_text(text);
    },
    ...
    _updateAltCal: function(now) {
        ...
        if (this.show_julian)  lines.push(_("Julian date")  + ": " + Calendars.formatJulian(y, m, d));
