...
gulp.task('install', (done) => {
    ...
    const systemXletDir = `${homeDir}/.local/share/cinnamon/desklets/${UUID}/`;
    ...
    if (uid !== userInfo.uid || gid !== userInfo.gid) {
        throw new Error(`Incorrect permission are set for the desklets directory. Please run 'gulp help'.`);
    }
...
gulp.task('help', gulp.series('clear-terminal', (done) => {
    console.log(
        ...
        auto-reload the desklet on code change.
