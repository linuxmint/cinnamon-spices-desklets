...
CinampDesklet.prototype = {
    ...
    _init: function (metadata, desklet_id) {
        ...
        this.visTick = 0;
        this.visBars = 16;
        this.visPeak = new Array(this.visBars).fill(0.02); // Init with 2% floor
        this.visRmsData = 0.0;
        ...
        this.visUpdateRate = 0.05; // Default placeholder, overwritten by settings
    ...
    _onMusicDirChanged: function() {
        this._normalizeMusicDir();
        this._refresh(); // Stop player, rescan, rebuild UI
    },

    _onSettingsVolumeChanged: function() {
        // Optimization: Setting the volume on the player here handles external changes (e.g., settings dialog)
    ...
    /**
     * Refactored helper to create standardized buttons.
     * @param {string} iconName - Icon symbolic name.
     * @param {Function} callback - Function to run on click.
     * @param {string} tooltipText - Tooltip text.
     * @param {number} [size=0] - Optional size for small buttons (e.g., 16).
     * @returns {{btn: St.Button, icon: St.Icon, tooltip: Tooltips.Tooltip}}
     */
    _createBtn: function(iconName, callback, tooltipText, size = 0) {
        ...
        if (size > 0) {
            // Apply custom style for small buttons (like MinMode/Close)
    ...
    this.shuffleRef = this._createBtn('media-playlist-shuffle-symbolic', this._toggleShuffle, "Shuffle Off"); // Initial text
    this.repeatRef  = this._createBtn('media-playlist-repeat-symbolic', this._toggleRepeat, "Repeat Off"); // Initial text
    ...
    Mainloop.idle_add(Lang.bind(this, function() {
        if (this.scroll.vscroll && this.scroll.vscroll.adjustment) {
            // This connection enables the _updatePlaylistReactivity optimization
    ...
    _restartVisualizerLoop: function() {
        // Called when settings change to update the timing immediately
        if (this.showVisualizer) {
            this._startVisualizerLoop();
        }
    },
    ...
    _startVisualizerLoop: function() {
        // Optimization: Ensure old timeout is removed before setting new one
        if (this.visTimeout) Mainloop.source_remove(this.visTimeout);

        // Convert seconds from settings (0.01 - 0.1) to milliseconds (10 - 100)
        let interval = Math.floor(this.visUpdateRate * 1000);

        // Sanity check: Clamp at 20ms (50fps) to prevent CPU hogging if setting is weird
  ...
  _onDrawVisualizer: function(area) {
        ...
        for (let i = 0; i < bars; i++) {
            ...
            // 1. Enforce Minimum Floor (2% Full)
            ...
            const R = 100/255; // 0.39
            const G = 181/255; // 0.71
            const B = 246/255; // 0.96

            // Bottom: More Solid (0.9 alpha)
            pat.addColorStopRGBA(1, R, G, B, 0.9);
            // Top: More Transparent (0.5 alpha)
    ...
    _updateMinimalModeUI: function() {
        ...
        /*  this.volumeSlider.add_style_class_name('cinamp-minimal-slider');
            this.progressBar.add_style_class_name('cinamp-minimal-slider'); */

            this.controlsBox.vertical = false;
            this.controlsBox.style_class = 'cinamp-controls-box minimal';  // For CSS
            this.playerSection.style_class = 'cinamp-player-section minimal';
            this.box.style_class = 'cinamp-container minimal';

            // Reorder for better flow: status (expand), buttonBox (no progress or volume)
            ...
            // Add minimal class to topBar for styling
            ...
            // Remove minimal class from topBar
        ...
        this._ensureDeskletOnScreen(); // Added to check/adjust position after size/layout change
    },
    ...
    _updatePlaylistReactivity: function() {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (btn instanceof St.Button) {
                ...
                // Check if the actor is vertically within the current viewport
   ...
   _reorientPlaylist: function() {
        ...
        if (!this.showPlaylist) {
            ...
            this._ensureDeskletOnScreen(); // Check position when playlist hides (shorter desklet)
            return;
        }
        ...
        // Use an idle_add to ensure layout calculations are complete before checking the position
        Mainloop.idle_add(Lang.bind(this, function() {
            this._updatePlaylistReactivity();
            this._ensureDeskletOnScreen(); // Check position after the playlist is re-added and re-laid out
            return false;
        }));
    },


    // Ensures the desklet is fully visible on screen, moving it up if necessary.
    ...
    _ensureDeskletOnScreen: function() {
        ...
        // Calculate the bottom position of the desklet
        let deskletBottom = y + height;
        
        // Check if the bottom is off-screen
        if (deskletBottom > screenHeight) {
            // Calculate new Y position, ensuring it doesn't go off the top edge (y >= 0)
            let newY = Math.max(0, screenHeight - height);
            
            if (newY < y) {
                // Only move if the new position is higher than the current one
                this.actor.set_position(x, newY);
            }
        }
    },
    ...
    _scanMusicDir: function () {
        ...
        if (this.playlist.length === 0) {
            ...
            this.currentIdx = 0;
            this._updateStatusAndScroll();
            // Load the first track's metadata and art, but do not play it yet.
   ...
   _updateInfoLabel: function() {
    ...
    _loadAlbumArtForCurrent: function (isNewTrack = false) {
        ...
        try {
            // Pass the isNewTrack flag to the discovery operation
            this.discoverer.discover_uri_async(uri, isNewTrack);
            
            // Set temporary status with filename before discovery completes
            this.currentTrackArtist = '';
    ...
    _onDiscovered: function (discoverer, info, error) {
        // 1. Handle Errors/Missing Info
        if (error || !info || !info.get_tags()) {
            this.albumArt.icon_name = 'audio-x-generic';
            this.currentTrackArtist = '';
            // Retain this.currentTrackTitle (filename fallback)
            this._updateStatusAndScroll(); // Update to show error or just filename
            return;
        }

        let tags = info.get_tags();

        // 2. Extract and Update Track Metadata (FIX for "missing track information")
        ...
        try {
            ...
            this.albumArt.icon_name = null; // Clear icon_name just in case
    ...
    /**
     * Handles the two-step process of loading a new track: Discover (metadata/art) -> Play
     * @param {number} idx - The index of the track to start.
     * @param {boolean} seekToStart - If true, seeks to 0 after playing (used for Repeat One).
     */
    _startNewTrack: function (idx, seekToStart) {
        ...
        // 1. Start discovery (updates status with initial filename, then with tags)
    ...
    _prevTrack: function () {
        ...
        if (this.repeat === 2) {
            this._startNewTrack(this.currentIdx, true); // Replay current
            return;
        }
        if (this.shuffle) {
    ...
    _togglePlayPause: function () {
        ...
        else this._startNewTrack(this.currentIdx, false); // Start new track if stopped
    },
    ...
    _stop: function () {
        ...
        // Keep discoverer running until a new track is explicitly started (better)
        if (this.discoverer) {
    ...
    _playCurrent: function (seekToStart = false) {
        ...
        // Custom audio-sink pipeline required to inject the 'level' element for visualization
        ...
        this.levelElement.set_property("interval", 80000000); // 80ms interval
        ...
        // Optional seek to start (used for repeat one, after starting playback)
        if (seekToStart) {
    ...
    _onBusMessage: function (bus, message) {
        if (message.type === Gst.MessageType.EOS) {
            ...
            if (struct && struct.get_name() === "level") {
                ...
                // Match the content inside < ... > for GValueArray (Stereo/Multi-channel)
                ...
                if (match && match[1]) {
                    ...
                    // Fallback: Try parsing single value (Mono)
                ...
                if (dataArray.length > 0) {
                    ...
                    for (let db of dataArray) {
                        // Convert dB (negative) to linear amplitude (0.0 to 1.0)
                        if (db < -100) sum += 0;
    ...
    _trackFinished: function () {
        this._stop(); // Clear current player

        if (this.repeat === 2) {
            // Repeat One: Replay the current track without changing index
            this._startNewTrack(this.currentIdx, true);
        } else if (this.repeat === 1 || this.shuffle || this.currentIdx < this.playlist.length - 1) {
            // Repeat All, Shuffle, or not the end of the playlist
            if (this.shuffle) {
                this.currentIdx = Math.floor(Math.random() * this.playlist.length);
            } else {
                this.currentIdx = (this.currentIdx + 1) % this.playlist.length;
            ...
            this.currentIdx = 0;
            this._updateStatusAndScroll(); // Update status to reflect stopped state at index 0
        }
    },
