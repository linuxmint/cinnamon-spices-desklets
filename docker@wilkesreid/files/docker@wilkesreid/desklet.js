...
DockerManagerDesklet.prototype = {
  ...
  _buildLayout: function () {
    ...
    this._mainBox.add(this._scrollView, {
      expand: true,
      x_fill: true,
      y_fill: true,
    });
  ...
  _renderContainers: function (groups, allContainers) {
    ...
    this._countLabel.set_text(
      runningCount + "/" + allContainers.length + " running",
    );
    ...
    for (let g = 0; g < groups.length; g++) {
      ...
      if (group.name) {
        ...
        let standaloneHeader = new St.BoxLayout({
          style_class: "docker-group-header",
        });
  ...
  _runDockerAction: function (action, ids) {
    ...
    try {
      ...
      this._stopSpinner();
      global.logError(
        UUID + ": Failed to run docker " + action + ": " + e.message,
      );
      this._fetchContainers();
    }
  },
