...
const UUID = "watt-watcher@drstevendev";
...
MyDesklet.prototype = {
  ...
  _init: function (metadata, desklet_id) {
    ...
    this.settings.bindProperty(
      ...
      "refresh-interval",
      "refresh_interval",
      Lang.bind(this, this._on_setting_changed)
    );
    this.settings.bindProperty(
      Settings.BindingDirection.IN,
      "power-source",
  ...
  _detectBattery: function () {
    try {
      let powerSupplyDir = Gio.file_new_for_path("/sys/class/power_supply");
      let enumerator = powerSupplyDir.enumerate_children("standard::name", Gio.FileQueryInfoFlags.NONE, null);
      let batFiles = [];
      let acFiles = [];
      let info;
      try {
        while ((info = enumerator.next_file(null)) !== null) {
          ...
          if (upperName.startsWith("BAT")) {
            batFiles.push(name);
          } else if (
            upperName.startsWith("AC") ||
            upperName.startsWith("ADP") ||
            upperName.startsWith("ACAD") ||
            upperName.startsWith("ACPI")
          ) {
            acFiles.push(name);
          }
        }
      ...
      if (batFiles.length > 0 && !batFiles.includes(this.powerSourceId)) {
        this.powerSourceId = batFiles[0];
        log(`Configured battery not found, switching to ${this.powerSourceId}`);
      }

      if (acFiles.length > 0) {
        if (!this.acSupplyName || !acFiles.includes(this.acSupplyName)) {
          this.acSupplyName = acFiles[0];
        }
      } else {
        this.acSupplyName = null;
      }
    } catch (e) {
      logError(e, "Error detecting power supplies");
      this.acSupplyName = null;
    }
  },
  ...
  _updateFilePaths: function () {
    if (this.powerSourceId) {
      ...
      this.fullEnergyFile = null;
      this.powerNowFile = null;
    }

    if (this.acSupplyName) {
      this.isChargingFile = Gio.file_new_for_path(
        `/sys/class/power_supply/${this.acSupplyName}/online`
      );
    } else {
      this.isChargingFile = null;
    }
  },
  ...
  setCapacity: function () {
    try {
      let [success, contents, tag] = this.capacityFile.load_contents(null);
      if (success) {
        let capacity = ByteArray.toString(contents);
        this.capacity = parseInt(capacity);
      }
    } catch (e) {
      logError(e, "Error reading capacity file");
    }
  },

  setEnergyFull: function () {
    try {
      let [success, contents, tag] = this.fullEnergyFile.load_contents(null);
      if (success) {
        let energyFull = ByteArray.toString(contents);
        this.energyFull = parseInt(energyFull) / 1000 / 1000;
      }
    } catch (e) {
      logError(e, "Error reading energy full file");
    }
  },

  setIsCharging: function () {
    try {
      let [success, contents, tag] = this.isChargingFile.load_contents(null);
      if (success) {
        let isCharging = ByteArray.toString(contents);
        this.isCharging = parseInt(isCharging);
      }
    } catch (e) {
      logError(e, "Error reading charging status file");
    }
  },

  setPowerNow: function () {
    try {
      let [success, contents, tag] = this.powerNowFile.load_contents(null);
      if (success) {
      ...
      logError(e, "Error reading power now file");
    }
  },
